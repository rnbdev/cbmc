...
Function: replace_symbol_sett::replace_symbol_sett
...
replace_symbol_sett::replace_symbol_sett()
{
}
...
Function: replace_symbol_sett::insert
...
void replace_symbol_sett::insert(
  const symbol_exprt &old_expr,
  const exprt &new_expr)
{
  // expr_map.insert(std::pair<irep_idt, exprt>(
  //   old_expr.get_identifier(), new_expr));
  expr_map[old_expr.get_identifier()].insert(new_expr);
}

/*******************************************************************\

Function: replace_symbol_sett::replace
...
bool replace_symbol_sett::replace(exprt &dest) const
{
  bool result=true;
...
Function: replace_symbol_sett::have_to_replace
...
bool replace_symbol_sett::have_to_replace(const exprt &dest) const
{
  // first look at type
...
bool replace_symbol_sett::replace(typet &dest) const
{
  if(!have_to_replace(dest))
    return true;
...
bool replace_symbol_sett::have_to_replace(const typet &dest) const
{
  if(dest.has_subtype())
